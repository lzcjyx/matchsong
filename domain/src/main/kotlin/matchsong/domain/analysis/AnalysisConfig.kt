...
object AnalysisConfig {
    ...
    // ---------- BUG-015 语音干扰门禁 [推测]（真机数据标定） ----------

    /**
     * 稳定片段比例下限：稳定帧/有效帧 < 该值判定为「非歌唱语音为主」
     * （说话语调连续滑动，稳定帧占比远低于演唱的持续稳定音），
     * 此时拒绝生成正式音域结果（按数据不足处理，FR-ANAL-8）。
     * 保守默认 0.3：演唱（持续音多）通常 ≥ 0.6，连续说话通常 < 0.2 [推测]。
     */
    const val MIN_STABLE_FRAME_RATIO = 0.3
