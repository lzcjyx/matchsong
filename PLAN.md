...
**状态：** `DONE`（2026-07-31，语言/排除风格/数据完整性/可信度/可调音域过滤；无性别硬过滤）
...
**状态：** `DONE`（2026-07-31，±6 半音枚举 + 变调后音域 + 优先降调策略 + 不可调标记）
...
**状态：** `DONE`（2026-07-31，六特征评分 + 权重 v1 集中配置 + 置信度调整 + 确定性排序）
...
**状态：** `DONE`（2026-07-31，模板+实际数据填充，无数据不生成；一致性测试 ACC-16）
...
**状态：** `DONE`（2026-07-31，空状态原因 + 建议 + 不伪造高分；LOW 置信短路）
...
**状态：** `DONE`（2026-07-31，10 场景测试 + 可重复性 + 装配用例（GetRecommendationsUseCase + ViewModel））
...
**状态：** `DONE`（2026-07-31，全流程编排：录音落盘→质量自动检测→分析→结果；取消/重录/防重复；Activity 重建恢复）
...
**状态：** `DONE`（2026-07-31，声音结果→真实推荐引擎→列表（分数/解释/变调）→详情→重测）
...
**状态：** `DONE`（2026-07-31，数据层（DAO/仓库/用例）+ 详情页按钮 + 收藏列表页，Flow 实时同步）
...
**状态：** `DONE`（2026-07-31，analysis_history 表（不含音频）+ 迁移 v1→v2 + 列表/删除 UI）
...
**状态：** `DONE`（2026-07-31，六类反馈表（v2→v3 迁移）+ 底部选择 UI，仅保存不调权重）
...
**状态：** `DONE`（2026-07-31，ErrorRecoveryHandler 错误→文案+动作映射，十类场景）
...
**状态：** `DONE`（2026-07-31，MainFlowE2eTest 5 场景：Onboarding→首页→准备→设置→删除确认→历史/收藏可达；18/18 仪器测试）
