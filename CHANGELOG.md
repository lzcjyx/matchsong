...
- **BUG-022/023 第二轮修复（2026-08-01，子代理调查 + 数据计算）**
  - BUG-022（P1）：成功后返回残留 QUALITY_RESULT 误报"文件不可用"（分析后清空流程栈，返回直达主菜单）；二次测试直接"完成"卡死（会话改录音页进入时启动，权限回调只改状态）；历史记录空（RecordAnalysisUseCase 零调用方 → 接线到 RecommendationListViewModel）。
  - BUG-023（P1）：推荐频繁全空——KeyShiftEvaluation 二进制全包含判定在偏女调数据集上滤掉典型男声；改两级匹配（零超幅 → 部分重叠 ≥0.6/超幅 ≤4）；数据集补 2 首男调歌（52 首 v1.1.0，支持旧内置版本升级）；实测男声 48-60 通过数 0 → 27/32。
  - 验证：新增部分重叠场景测试；仪器 24/24；全门禁绿。
